...
describe("ViewHolderCollection Opacity", () => {
  ...
  it("should set opacity to 1 when containerLayout is available", () => {
    // This test verifies that the opacity is set to 1 when the containerLayout exists,
    // which fixes the issue where opacity stays at 0 when the component becomes visible
    // again in stack navigation scenarios.
    const result = render(
      <FlashList
        data={[0, 1, 2, 3, 4]}
  ...
  it("should render with opacity 1 even on remount with layout available", () => {
    // This simulates the stack navigation scenario where a component
    // is unmounted and remounted with existing layout
    ...
    // The remounted component should still render items with opacity 1
    expect(result).toContainReactComponent(Text, { children: 0 });
    expect(result).toContainReactComponent(Text, { children: 1 });
  });
});
